/**
 * @file Data-page preview/publish/revoke UI for immutable Community snapshots
 * (issue #253). This is owner-only technical tooling, distinct from the live
 * Supabase public-profile switch on the same page: the browser previews the
 * exact public projection (window.publicProjectionPreview, already built for
 ...
 *
 * The slug and display name are not typed here: both come from the Profile
...
/** Builds a Community-snapshot publication preview from the current settings. */
...
window.handlePublicProfilePublicationAction = function (event) {
  ...
  if (event.target.closest("[data-public-profile-preview]")) {
    ...
    if (
      ...
      !window.confirm(
        "Prepare to remove every Community snapshot? Nothing changes online until the publishing workflow runs. Existing snapshot files remain in repository history and can be published again.",
      )
    )
      return;

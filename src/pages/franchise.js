 ...
 * actions (entity note, watchlist tier/order edits, local rank) route to
...
(function () {
  ...
  function render() {
    ...
    let implicitFilmIds = localRankEntries.map(
      (item) => item.film.supabaseFilmId,
    );
    let filmEntries =
      filmSort === "local"
        ? [...localRankEntries].sort((left, right) => {
            let leftPos =
              localRankMap.get(left.film.supabaseFilmId) ?? Infinity;
            let rightPos =
              localRankMap.get(right.film.supabaseFilmId) ?? Infinity;
            return filmOrder === "desc"
              ? rightPos - leftPos
              : leftPos - rightPos;
          })
        : [...entries].sort(franchiseCompare);
    let otherFilmEntries = (filmSort === "local" ? [] : [...otherEntries]).sort(
      (left, right) => {
        if (filmSort === "rank")
          return (
    ...
    // Takes the whole film (not just an id) because the two backing maps
    // use different id schemes: franchiseRank is keyed by film.id (the
    // legacy composite id, matching filmPageUrl()'s own routing since
    // film.html hasn't cut over to Supabase yet), while localRankMap is
    // keyed by film.supabaseFilmId (local_ranks.film_id is a real uuid FK
    // - the legacy composite id isn't a valid value there at all).
    function franchiseFilmPosition(film) {
      return filmSort === "local"
        ? localRankMap.get(film.supabaseFilmId) || "—"
        : franchiseRank.get(film.id) || "—";
    }
    function franchiseFilmRow({ entry, film }, index = 0) {
      ...
      return `<tr${attributes}><td class="leaderboard-position">${franchiseFilmPosition(film)}</td>${window.renderFilmIdentityCell(
        film,
        {
          escape,
          year: true,
    ...
    function filmCard(entryRecord, index = 0) {
      ...
      return window.renderSharedFilmCard(film, {
        ...
        compare: false,
        showYear: true,
        rankLabel: `${franchiseFilmPosition(film)}.`,
        escape,
    ...
    let nextWatchlist = upNextItems.length
      ? `<section class="franchise-next-watch"><h2>${escape(ui(upNextItems.length > 1 ? "Up next" : "Next unwatched"))}</h2><div class="franchise-next-watch-list">${upNextItems.map((item) => `<div class="film-table-cell">${window.renderFilmPoster(window.watchlistFilmLike(item), "thumb")}<span><a class="table-film-link" href="${escape(window.filmPageUrl(item.supabaseFilmId))}"><strong>${escape(window.localizedFilmTitle?.(window.watchlistFilmLike(item)) || item.title)}</strong></a><span class="leaderboard-meta">${escape(item.year || "")}${item.tier ? ` · ${escape(ui("Tier"))} ${escape(item.tier)}` : ""}</span></span></div>`).join("")}</div></section>`
      : "";
    ...
    window.createOrderEditController({
      ...
      commit: async (from, target, position) => {
        let fromRecord = watchlistEntries.find(
          (record) => record.item.id === from.id,
        );
        let toRecord = watchlistEntries.find(
          (record) => record.item.id === target.id,
        );
        ...
        let fromTier = window.normalizeWatchlistTier(fromRecord.item.tier);
        let toTier = window.normalizeWatchlistTier(toRecord.item.tier);
        if (fromTier !== toTier)
          return {
            ok: false,
            reason:
        ...
        let tierItems = watchlistEntries
          .filter(
            (record) =>
              window.normalizeWatchlistTier(record.item.tier) === fromTier,
          )
  ...
  async function boot() {
    ...
    window.bindSupabaseWatchlistBulkTierControl({
      container,
      entries: () =>
        (window.ensureFranchiseIndex()[franchiseId]?.watchlistFilms || [])
    ...
    try {
      ...
      if (franchise && filmSort === "local") {
        let allTimeOrderedEntries = franchise.films
          .map((entry) => ({ entry, film: state.filmsById[entry.filmId] }))
          .filter((item) => item.film)
          .sort((left, right) =>
            window.compareByAllTimeRank(left, right, (item) => item.film),
          );
        let otherOrderedEntries = (franchise.otherFilms || [])
          .map((entry) => ({
            entry,
            film: (state.watchedOther || []).find(
              (film) => film.id === entry.filmId,
        ...
        let implicitFilmIds = allTimeOrderedEntries
          .concat(otherOrderedEntries)
          .map((item) => item.film.supabaseFilmId);
        let stored = await window.loadSupabaseLocalRankOrder(
          "franchise",
          franchise.id,
        );
        let order = window.mergeSupabaseLocalRankOrder(stored, implicitFilmIds);
      ...
      container.innerHTML = `<section class="detail-empty"><h2>${escape(ui("Could not load this franchise"))}</h2><p>${escape(error.message || String(error))}</p></section>`;
    }
  }

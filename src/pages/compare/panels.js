...
function watchlistItemLinkList(items) {
  ...
  return items
    .map((item) => {
      ...
      let title = window.localizedFilmTitle?.(film) || item.title || film.title;
      let meta = [
...
window.compareRenderMetricsPanel = function (films) {
  let bundles = new Map(films.map((film) => [film.id, filmMetricBundle(film)]));
  let rows = [
...
window.compareRenderOverviewPanel = function (targets) {
  let metricKeys = [
    ...new Set(targets.flatMap((target) => Object.keys(target.metrics || {}))),
  ];
  let rows = [
...
window.compareRenderActionPanel = function (targets) {
  ...
  return comparePanel(
    "Actions",
    compareTable([ui("Target"), ui("Links"), ui("Project")].map(escape), rows),
    { introHtml },
  );
};
...
function filmPersonRoles(filmTarget, personTarget) {
  ...
  let directorIds = normalizeList(
    ...
    (name) => window.normalizePersonName?.(name) || window.normalizeTitle(name),
  );
...
function filmPeriodContext(filmTarget, periodTarget) {
  let entry = (periodTarget.record?.films || []).find(
    (candidate) => (candidate.id || candidate.filmId) === filmTarget.record.id,
  );
  ...
  if (Number(entry.rank) > 0) parts.push(`${ui("Rank")} #${entry.rank}`);
...
function relationshipContext(left, right, overlap) {
  ...
  if (filmTarget && personTarget) {
    ...
    return roles.length
      ? roles.join(", ")
      : ui("No credited role in this film");
  }
...
window.compareRenderCollectionDistributionPanel = function (targets) {
  ...
  return comparePanel(
    ...
    compareTable(
      [ui("Target"), ui("Ranked"), ui("Top 250"), ui("Rating"), ui("Tier")].map(
        escape,
      ),
      rows,
    ),
...
window.compareRenderPeriodProfilePanel = function (targets) {
  ...
  let rows = targetEntries
    .map(({ target, entries }) => {
      ...
      let density = filmCount ? (stats.awardScore / filmCount).toFixed(1) : "—";
      return `<tr>
      ${targetHeaderCell(target)}

...
window.showImportReport = function (report) {
  ...
  body.innerHTML = `
    ...
    ${proposalChangeRows ? `<div class="import-report-proposal-changes"><b>${importReportEscape(ui("Changes to your archive"))}</b><div class="leaderboard-wrap"><table class="leaderboard import-proposal-change-summary"><thead><tr><th>${importReportEscape(ui("Part of archive"))}</th><th>${importReportEscape(ui("Changed records"))}</th><th>${importReportEscape(ui("Before"))}</th><th>${importReportEscape(ui("After"))}</th></tr></thead><tbody>${proposalChangeRows}</tbody></table></div></div>` : ""}
    ${letterboxdSummary ? `<div class="import-report-letterboxd-summary"><b>${importReportEscape(ui("Letterboxd jumpstart"))}</b><p>${letterboxdSummary.watchedArchiveMerged || 0} archive film(s) matched${letterboxdSummary.archiveAdded ? ` · ${letterboxdSummary.archiveAdded} film(s) added unranked` : ""} · ${letterboxdSummary.watchedOtherAdded || 0} standalone watched film(s) added · ${letterboxdSummary.watchedOtherMerged || 0} standalone watched film(s) merged · ${letterboxdSummary.watchlistAdded || 0} watchlist item(s) added · ${letterboxdSummary.watchlistMerged || 0} watchlist item(s) merged · ${letterboxdSummary.watchlistRemoved || 0} watched watchlist item(s) removed</p><p class="import-report-note">${importReportEscape(letterboxdSummary.archiveAdded ? ui("Awards are not part of this import. Newly added films have no rank yet - rank each year to include them in that year's awards.") : ui("Awards and rankings are not part of this import."))}</p></div>` : ""}
